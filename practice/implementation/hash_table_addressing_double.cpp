...
struct HashTable {
    ...
    long long hash1(const string &key) {
        long long hash = 0, base = 31;
        for (char c : key) hash = (hash * base + c) % size;
        return hash % size;
    }

    long long hash2(const string &key) {
        long long hash = 0, base = 37;
        for (char c : key) hash = (hash * base + c) % (size - 1);
        return (hash % (size - 1)) + 1; // шаг != 0
    }

    void insert(const string &key, int value) {
        long long h1 = hash1(key);
        long long h2 = hash2(key);
        int first_deleted = -1;
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (table[index].deleted && first_deleted == -1) first_deleted = index;
        }
